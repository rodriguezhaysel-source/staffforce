import type { TimeEntry, Employee, PayrollEntry } from '../types'
...
export function calculateHours(entries: TimeEntry[], config?: Partial<PayrollConfig>): { regular: number; overtime: number; total: number } {
  const cfg = { ...DEFAULT_PAYROLL_CONFIG, ...config }
  const byDay: Record<string, number> = {}
  for (const entry of entries) {
    if (!entry.clock_in || !entry.clock_out) continue
    const ms = new Date(entry.clock_out).getTime() - new Date(entry.clock_in).getTime() - (entry.break_mins || 0) * 60000
    if (ms <= 0) continue
    const day = entry.clock_in.slice(0, 10)
    byDay[day] = (byDay[day] || 0) + ms / 3600000
  }
  let regularTotal = 0, overtimeTotal = 0
  for (const hrs of Object.values(byDay)) {
...
export function generateNACHA(entries: PayrollEntry[], employees: Employee[], periodId: string): string {
  ...
  lines.push('1' + '01'.padStart(2) + ' 123456789'.padEnd(10) + '1234567890'.padEnd(10) + fileDate + fileTime + 'A' + '094' + '10' + '1'.padStart(7, '0') + 'STAFFFORCE'.padEnd(23) + 'STAFFFORCE PAYROLL  ' + '1')
  let entrySeq = 1, totalDebit = 0
  lines.push('5' + '200' + 'PAYROLL   '.padEnd(10) + ''.padEnd(3) + 'STAFFFORCE PAYRL'.padEnd(16) + 'PPD' + now.toISOString().slice(2, 10).replace(/-/g, '') + '   ' + '1' + '123456789' + '1'.padStart(7, '0'))
  for (const entry of entries) {
    ...
    lines.push('6' + '22' + routing + account + amount + name + '  ' + '0' + '123456789' + String(entrySeq).padStart(7, '0'))
    entrySeq++
  }
  const ec = entrySeq - 1
  lines.push('8' + '200' + String(ec).padStart(6, '0') + '0000000000' + String(totalDebit).padStart(12, '0') + 'STAFFFORCE'.padEnd(23) + ''.padEnd(39) + '123456789' + '1'.padStart(7, '0'))
  const blockCount = Math.ceil((lines.length + 1) / 10)
  lines.push('9' + '000001' + String(blockCount).padStart(6, '0') + String(ec).padStart(8, '0') + '0000000000' + String(totalDebit).padStart(12, '0') + ''.padEnd(39))
