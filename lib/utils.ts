...
// Strip sensitive fields from employee records before storing in React state
export function sanitizeEmployee(emp: any): any {
  if (!emp) return emp
  const { password_hash, bank_account, bank_routing, bank_name, bank_account_type, internal_notes, ...safe } = emp
  return safe
}

export function sanitizeEmployees(emps: any[]): any[] {
  return (emps || []).map(sanitizeEmployee)
}

// Safe query helper — logs errors instead of silently swallowing them
export async function safeQuery<T>(promise: PromiseLike<{ data: T | null; error: any; count?: number | null }>): Promise<{ data: T | null; error: any; count?: number | null }> {
  const result = await promise
  if (result.error) {
    console.error('[StaffForce DB Error]', result.error.message || result.error)
  }
  return result
}
