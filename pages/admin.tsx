              ...
              return <tr key={dept.id}>
                ...
                {days.map(d => {
                  ...
                  return <td key={d.toISOString()} style={{ padding: 3, border: `1px solid ${C.border}`, background: isToday(d) ? '#F0F7FF' : '#fff', verticalAlign: 'top' }}>
                    {ds.map(s => { const emp = employees.find(e => e.id === s.employee_id); const draft = s.status === 'draft'; return <div key={s.id} style={{ padding: '3px 6px', borderRadius: 5, marginBottom: 2, fontSize: 10, background: draft ? 'transparent' : dept.color + '18', color: dept.color, border: draft ? `1.5px dashed ${dept.color}` : `1px solid ${dept.color}30` }}>
                      <div style={{ fontWeight: 800 }}>{format(new Date(s.start_time),'HH:mm')}-{format(new Date(s.end_time),'HH:mm')}</div>
                      <div style={{ opacity: 0.85 }}>{emp ? emp.first_name + ' ' + (emp.last_name?.[0]||'') + '.' : '-'}</div>
                      {draft && <div style={{ fontSize: 8, opacity: 0.7, fontWeight: 700 }}>DRAFT</div>}
                    </div>})}
                  </td>
                })}
              </tr>
