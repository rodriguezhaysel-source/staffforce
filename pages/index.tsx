...
function LoginScreen({
  ...
  const handleRecovery = async () => {
    ...
    setRecoveryMsg('')
    try {
      const cleanRecEmail = recoveryEmail.trim().toLowerCase()
      const cleanRecPass = recoveryPass.trim()
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: empList, error: empErr } = await supabase
        ...
        .eq('email', cleanEmail)
        .eq('status', 'active')
      if (empErr || !empList || empList.length === 0) throw new Error(t(lang, 'invalidCredentials'))
      const emp = empList[0]
      if (!emp.password_hash || emp.password_hash.trim() !== cleanPassword) throw new Error(t(lang, 'invalidCredentials'))
      onLogin(emp)
    } catch (err: unknown) {
