...
function applyTheme(theme: ThemeConfig) {
  ...
  Object.entries({
    ...
    '--v-dim': theme.v + '26',
    '--v-glow': theme.v + '66',
    '--success-dim': theme.success + '26',
    '--warning-dim': theme.warning + '26',
    '--danger-dim': theme.danger + '26',
    '--info-dim': theme.info + '26',
...
function LoginScreen({
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authErr || !data.user) throw new Error(t(lang, 'invalidCredentials'))
      const { data: emp } = await supabase
        ...
        .eq('status', 'active')
        .single()
      if (!emp) throw new Error(t(lang, 'invalidCredentials'))
      onLogin(emp)
    } catch (err: unknown) {
...
function TimesheetsModule({ user, lang, addToast }: { user: Employee; lang: Lang; addToast: (type: Toast['type'], msg: string) => void }) {
  ...
  const loadData = async () => {
    ...
    if (user.role !== 'employee') {
      const { data: emps } = await supabase.from('employees').select('id, first_name, last_name').eq('status', 'active').order('first_name')
      setEmployees(emps || [])
    }
    setLoading(false)
  }
...
function DocumentsModule({ user, lang, addToast }: { user: Employee; lang: Lang; addToast: (type: Toast['type'], msg: string) => void }) {
  ...
  const loadData = async () => {
    ...
    const [{ data: docs }, { data: emps }] = await Promise.all([
      q,
      user.role !== 'employee' ? supabase.from('employees').select('id, first_name, last_name').eq('status', 'active') : Promise.resolve({ data: [] }),
    ])
