                ...
                {postings.map(p => {
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td><span className={`badge ${p.status === 'open' ? 'badge-success' : 'badge-gray'}`}>{p.status}</span></td>
          ...
          {STAGES.map(stage => (
            <div key={stage} className="kanban-col">
              <div className="kanban-col-header">
                ...
                <span className="badge badge-gray">{candidates.filter(c => c.stage === stage).length}</span>
              </div>
                ...
                {candidates.filter(c => c.stage === stage).map(c => {
                  ...
                  return (
                    <div key={c.id} className="kanban-card">
                      ...
                      {post && <div style={{ fontSize: 10, color: 'var(--primary)', marginTop: 4 }}>{post.title}</div>}
