          ...
          <div key={l.id} className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>{l.address}</div>
              </div>
              <button className="btn btn-secondary btn-sm" onClick={() => openEdit(l)}><Icon name="edit" size={13} /></button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 12 }}>
              <div><span style={{ color: 'var(--text-secondary)' }}>Lat: </span><span className="mono">{l.lat?.toFixed(5)}</span></div>
              <div><span style={{ color: 'var(--text-secondary)' }}>Lng: </span><span className="mono">{l.lng?.toFixed(5)}</span></div>
              <div><span style={{ color: 'var(--text-secondary)' }}>Radius: </span><strong>{l.radius_meters}m</strong></div>
              <div><span style={{ color: 'var(--text-secondary)' }}>TZ: </span>{l.timezone}</div>
              <div><span style={{ color: 'var(--text-secondary)' }}>Currency: </span>{l.currency}</div>
            </div>
