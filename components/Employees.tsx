...
function EmployeeCard({ emp, dept }: { emp: Employee; dept?: Department }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <div style={{ width: 36, height: 36, borderRadius: '50%', background: avatarUrl ? 'transparent' : (dept?.color || 'var(--color-accent)'), overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid var(--color-border)' }}>
    ...
    <div>
              ...
              {filtered.map(e => {
                ...
                return (
                  <tr key={e.id} style={{ cursor: 'pointer' }} onClick={() => openEmp(e)}>
                    ...
                    <td><span className={`badge ${e.status === 'active' ? 'badge-success' : e.status === 'pending_activation' ? 'badge-warning' : 'badge-neutral'}`}>{e.status?.replace('_', ' ')}</span></td>
                  </tr>
                )
              })}
            </tbody>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={selectedEmp ? empName(selectedEmp) : ''} size="modal-lg">
        <div className="modal-body">
          {/* Profile header */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20, padding: '16px 0', borderBottom: '1px solid var(--color-border)' }}>
            <div style={{ position: 'relative' }}>
              <div style={{ width: 64, height: 64, borderRadius: '50%', background: (editData as any).avatar_url ? 'transparent' : 'var(--color-accent)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '3px solid var(--color-border)' }}>
              ...
              {canEdit && (
                <label style={{ position: 'absolute', bottom: 0, right: 0, background: 'var(--color-accent)', borderRadius: '50%', width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: '2px solid white' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ color: 'var(--color-text-secondary)', fontSize: 13 }}>{selectedEmp?.position}</div>
              <div style={{ marginTop: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <div style={{ fontSize: 11, color: 'var(--color-text-secondary)' }}>Profile {profileProgress}% complete</div>
                </div>
            ...
            <div style={{ display: 'flex', gap: 8, flexDirection: 'column', alignItems: 'flex-end' }}>
              ...
              {inviteMsg && <span style={{ fontSize: 12, color: inviteMsg.includes('Error') ? 'var(--color-danger)' : 'var(--color-success)', fontWeight: 600 }}>{inviteMsg}</span>}
            </div>
          </div>
          ...
          {tab === 1 && (
            <div>
              ...
              {bankAccounts.map((ba, i) => (
                <div key={ba.id} style={{ background: 'var(--color-bg)', borderRadius: 8, padding: '14px 16px', marginBottom: 10, border: '1px solid var(--color-border)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 13 }}>{ba.nickname || ba.bank_name} <span style={{ fontSize: 11, textTransform: 'capitalize', color: 'var(--color-text-secondary)' }}>({ba.account_type})</span></div>
                      <div style={{ fontSize: 12, color: 'var(--color-text-secondary)', marginTop: 2 }}>****{ba.account_number?.slice(-4)} | Routing: {ba.routing_number}</div>
      ...
      <Modal open={exitSurveyOpen} onClose={() => setExitSurveyOpen(false)} title="Exit Survey">
        <div className="modal-body">
          <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', marginBottom: 16 }}>Before deactivating {selectedEmp?.first_name}, please complete the exit survey.</p>
