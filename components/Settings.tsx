...
export default function Settings({ user, lang, employees, departments, setEmployees }: Props) {
  ...
  function applyTheme(th: typeof theme) {
    ...
    r.style.setProperty('--primary', th.accent)
    r.style.setProperty('--color-sidebar', th.sidebar)
    r.style.setProperty('--success', th.success)
    r.style.setProperty('--danger', th.danger)
    r.style.setProperty('--warning', th.warning)
  }
  ...
  const ColorRow = ({ label, key: k }: { label: string; key: keyof typeof theme }) => (
    <div className="form-group">
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <input type="color" value={theme[k]} onChange={e => { const v = e.target.value; setTheme(th => ({ ...th, [k]: v })); document.documentElement.style.setProperty(`--color-${k === 'bg' ? 'bg' : k === 'accent' ? 'accent' : k === 'sidebar' ? 'sidebar' : k}`, v) }} style={{ width: 44, height: 36, padding: 2, border: '1px solid var(--border)', borderRadius: 4 }} />
  ...
  const SectionTitle = ({ title }: { title: string }) => (
    <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 16, paddingBottom: 8, borderBottom: '2px solid var(--border)' }}>{title}</div>
  )
      ...
      <div style={{ maxWidth: 720 }}>
        ...
        {activeTab === 2 && (
          <div className="card">
            ...
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 20 }}>When Auto is enabled, the system handles the task automatically. Admin still reviews and approves before changes go live.</p>
            {(['scheduling','timesheets','payroll','timeoff'] as const).map(module => (
              <div key={module} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', borderBottom: '1px solid var(--border)' }}>
                <div>
                ...
                <div style={{ display: 'flex', border: '1px solid var(--border)', borderRadius: 6, overflow: 'hidden' }}>
                  <button onClick={() => setAutoCfg(a => ({ ...a, [module]: false }))} style={{ padding: '6px 16px', border: 'none', background: !autoCfg[module] ? 'var(--text-main)' : 'transparent', color: !autoCfg[module] ? '#fff' : 'var(--text-secondary)', fontWeight: 600, fontSize: 12, cursor: 'pointer' }}>Manual</button>
                  <button onClick={() => setAutoCfg(a => ({ ...a, [module]: true }))} style={{ padding: '6px 16px', border: 'none', background: autoCfg[module] ? 'var(--primary)' : 'transparent', color: autoCfg[module] ? '#fff' : 'var(--text-secondary)', fontWeight: 600, fontSize: 12, cursor: 'pointer' }}>Auto</button>
                </div>
              </div>
            ))}
        ...
        {activeTab === 3 && (
          <div className="card">
            ...
            <div className="form-group">
              ...
              <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 4 }}>Sick leave always bypasses this rule.</div>
            </div>
            <div className="form-group">
        ...
        {activeTab === 4 && (
          <div className="card">
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 20 }}>
              ...
              {testEmailMsg && <span style={{ fontSize: 13, color: testEmailMsg.includes('Error') ? 'var(--danger)' : 'var(--success)', fontWeight: 600 }}>{testEmailMsg}</span>}
            </div>
            ...
            {emailPreview && (
              <div style={{ marginTop: 16, border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                <div style={{ background: 'var(--color-bg)', padding: '8px 12px', fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)', borderBottom: '1px solid var(--border)' }}>EMAIL PREVIEW</div>
        ...
        {activeTab === 6 && (
          <div>
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16 }}>Choose which modules employees can see. Admins and Managers always have full access.</p>
                  ...
                  <tbody>
                    {employees.map(e => (
                      <tr key={e.id}>
                        ...
                        <td style={{ color: 'var(--text-secondary)', fontSize: 12 }}>{e.email}</td>
                        <td>
        ...
        <div style={{ marginTop: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {saved && <span style={{ color: 'var(--success)', fontWeight: 600, fontSize: 13 }}>{saved}</span>}
        </div>
      </div>
    </div>
  )
