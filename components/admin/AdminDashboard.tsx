...
export default function AdminDashboard({ user, employees, departments, locations }: Props) {
  ...
  useEffect(() => {
    let mounted = true
    load()
    const timer = setInterval(() => { if (mounted) { setNow(new Date()); load() } }, 30000)
    const ch = supabase.channel('adm-dash-rt')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'time_entries' }, () => { if (mounted) load() })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'shifts' }, () => { if (mounted) load() })
      .subscribe()
    return () => { mounted = false; clearInterval(timer); supabase.removeChannel(ch) }
  }, [])
