...
import { isWithinShift } from '../lib/colors'
import type { Employee, Location, TimeEntry, Shift } from '../lib/store'
...
export default function Clock({ user, lang, locations }: Props) {
  ...
  useEffect(() => {
    load()
    // Realtime: update if another session clocks in/out
    const ch = supabase.channel('clock-rt')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'time_entries', filter: `employee_id=eq.${user.id}` }, () => load())
      .subscribe()
  ...
  async function doClockIn(lat: number | null, lng: number | null, within: boolean) {
    ...
    else if (data) { setCurrent(data[0]); load() }
    setLoading(false)
  }
